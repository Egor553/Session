...
const App: React.FC = () => {
  ...
  const generateAdminSlots = async () => {
    ...
    if (success) {
      ...
      setAdminRange({ start: null, end: null });
       ...
       window.Telegram?.WebApp?.showAlert("Ошибка при сохранении в таблицу");
    }
    setActionLoading(false);
  };
        ...
        {days.map(date => {
          ...
          return (
            ...
            >
              ...
              {hasSlots && !selected && <div className="w-1 h-1 bg-blue-500 rounded-full mt-1" />}
              {selected && <div className="w-1 h-1 bg-white rounded-full mt-1" />}
            </button>
          );
        })}
      </div>
  ...
  if (isSuccess) return (
    <div className="h-screen flex flex-col items-center justify-center p-8 text-center bg-white animate-fade-in">
      <div className="w-24 h-24 bg-blue-500 rounded-[2.5rem] flex items-center justify-center text-white shadow-xl shadow-blue-100 mb-8 animate-bounce">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12" /></svg>
      </div>
      <h2 className="text-3xl font-black text-gray-900 tracking-tight mb-4">Запись готова!</h2>
      <div className="bg-blue-50 p-6 rounded-[2rem] border-2 border-blue-100 mb-8 w-full">
        <p className="text-blue-900 font-black text-sm uppercase tracking-widest mb-4">Напишите боту фразу:</p>
        <div className="bg-white p-5 rounded-2xl shadow-inner border border-blue-50 mb-4 select-all">
           <p className="text-3xl font-black text-blue-600 uppercase tracking-tighter">Готово</p>
        </div>
        <p className="text-gray-500 text-[10px] font-bold leading-relaxed uppercase text-center">Скопируйте слово выше и отправьте его боту</p>
      </div>
      <button 
        onClick={() => { navigator.clipboard.writeText("Готово"); window.Telegram?.WebApp?.close(); }} 
      ...
      {currentScreen === Screen.CITY_SELECT && (
        <div className="p-8 flex flex-col items-center justify-center flex-1 space-y-12">
          <div className="text-center space-y-6">
            ...
            <h1 className="text-4xl font-black text-gray-900 tracking-tighter">Ваш город?</h1>
          </div>
          <div className="w-full space-y-4">
            ...
            <button onClick={handleCitySearch} className="w-full py-5 bg-blue-500 text-white rounded-2xl font-black text-xl shadow-xl">Проверить</button>
          </div>
        </div>
      )}
        ...
        <div className="p-8 flex flex-col items-center justify-center flex-1 space-y-10 text-center">
          {isOfflineAvailable ? (
            <>
              ...
              <h2 className="text-2xl font-black text-gray-900">В городе {selectedCity} есть оффлайн сессии!</h2>
              <div className="w-full space-y-3">
                <button onClick={() => selectSessionType('offline')} className="w-full py-5 bg-blue-500 text-white rounded-2xl font-black text-lg shadow-xl">📍 Оффлайн сессия</button>
                <button onClick={() => selectSessionType('online')} className="w-full py-5 bg-white border-2 border-gray-100 text-blue-600 rounded-2xl font-black text-lg active:bg-gray-50 transition-all">🌐 Онлайн сессия</button>
              </div>
            </>
          ) : (
            <>
              ...
              <button onClick={() => selectSessionType('online')} className="w-full py-5 bg-blue-500 text-white rounded-2xl font-black text-lg shadow-xl">🌐 Записаться онлайн</button>
            </>
          )}
        ...
        <div className="flex flex-col flex-1 bg-white overflow-hidden">
          ...
          {selectedSlot && (
            <div className="p-6 bg-white border-t border-gray-100 fixed bottom-0 left-0 right-0 z-50 animate-fade-in shadow-2xl">
              <button onClick={() => setCurrentScreen(Screen.BOOKING_FORM)} className="w-full py-5 bg-blue-500 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-100 active:scale-95">Далее</button>
            </div>
          )}
        </div>
      )}
      ...
      {currentScreen === Screen.BOOKING_FORM && (
        <div className="flex flex-col flex-1 bg-white">
          ...
          <div className="p-8 space-y-6">
            ...
            <input type="tel" placeholder="Телефон" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full p-5 bg-gray-50 rounded-2xl font-black text-black outline-none border-2 border-transparent focus:border-blue-500" />
              ...
              <div className="bg-white p-6 rounded-[2.5rem] space-y-6 border border-gray-100 shadow-sm">
                ...
                <div className="space-y-4 p-5 border-2 border-black rounded-[2.5rem] bg-white text-black">
                  <div className="flex justify-between items-center mb-4">
                    <button onClick={() => changeMonth(-1)} className="p-3 bg-black text-white rounded-xl shadow-sm"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><path d="M15 19l-7-7 7-7" /></svg></button>
                    <span className="text-sm font-black text-black capitalize">{viewingMonth.toLocaleString('ru-RU', { month: 'long', year: 'numeric' })}</span>
                    <button onClick={() => changeMonth(1)} className="p-3 bg-black text-white rounded-xl shadow-sm"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><path d="M9 5l7 7-7 7" /></svg></button>
                  </div>
                  {renderCalendarGrid(
              ...
              <div className="space-y-8 pb-24 text-black">
                {Object.entries(adminGroupedSlots).length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 opacity-30">
                    ...
                    <p className="mt-4 font-black uppercase text-xs">Нет активных сессий</p>
                  </div>
                ) : (
